...
- **생성 히스토리** — 과거 생성 기록을 썸네일로 조회·다시보기·ZIP·삭제
  (모든 결과는 `OUTPUT_DIR` 아래 폴더로 자동 저장, UI 에서 경로 변경 가능)
- **애니메이션 프레임** — 대기/걷기/공격 프레임 시퀀스 → 가로 스프라이트
  스트립 + 반복 GIF 자동 생성 (2D 게임용)
- **타일셋(이음새 없음)** — 오프셋+시임 블렌드로 타일러블 처리 + 3×3 미리보기
- **프리셋 저장/불러오기** — 캐릭터 설정을 저장해 재사용
- **실시간 진행률 + 취소** — 생성 단계별 실제 진행률 표시, 진행 중 취소 가능
- **개별 에셋 재생성** — 결과에서 마음에 안 드는 에셋만 ↻ 로 다시 생성
- **이미지 편집** — 생성된 에셋을 ✎ 로 크롭·배경색 교체(투명 포함)·밝기/대비/채도 보정
- **변형 비교·채택** — 여러 변형(표정·포즈 등)을 ⊞ 로 나란히 비교, 선택한 것만
  남기고 스프라이트 시트를 다시 패킹
- **특수효과(VFX)** — 속성별(화염·빙결·전격·맹독·신성 등) 이펙트, 스킬 연출,
  타격 이펙트를 개수 선택해 생성
- **배경음악(BGM)** — 장르에 맞춘 루프 음원(WAV) + CapCut 음악 검색 가이드 생성
- **CSV / 스프레드시트 가져오기** — 표준 양식을 채워 올리면 **내용을 분석해
  필요한 아트를 자동 설정**(용도별 번들)하고 여러 개체를 한 번에 생성.
  `용도`(rpg·카드·2d·도감·프로필·ui) 만 적으면 에셋이 자동 선택됨. `.csv`/`.xlsx` 지원
- **예상 파일수·용량·비용** — 옵션에 따라 실시간 추정치를 상단에 표시
- **비용 확인·예산 경고** — 실 API 생성 전 예상 비용 확인 팝업, 1회 상한 /
  월 예산 초과 시 차단 (⚙예산에서 설정, 이번 달 실지출 누적 표시)
...
| 파일 | 기능 |
|------|------|
| **`start.bat`** | 서버를 **백그라운드(창 없이)** 실행 + 브라우저 자동 열림. 로그는 `server.log` |
| **`stop.bat`** | 서버 종료 (포트 8000 프로세스 정리) |
...
| `GET` | `/api/progress/{id}` | 생성 진행률 조회 (프론트 폴링) |
| `POST` | `/api/cancel/{id}` | 진행 중 생성 취소 |
| `POST` | `/api/regenerate/{job_id}` | 특정 에셋 하나만 다시 생성 |
| `POST` | `/api/edit/{job_id}` | 에셋 이미지 편집 (크롭·배경·보정) |
| `POST` | `/api/repack/{job_id}` | 변형 중 선택분만 남겨 시트 재패킹 |
| `GET` | `/api/template.csv` | 아트 제작 표준 양식 다운로드 |
| `POST` | `/api/import/preview` | CSV/xlsx 분석 → 생성 계획(미리보기) |
| `POST` | `/api/import/run` | 분석된 개체 목록 일괄 생성 |
| `GET` | `/api/budget` | 예산 상한 + 이번 달 실지출 |
| `POST` | `/api/budget` | 예산 상한 설정 (로컬 전용) |
