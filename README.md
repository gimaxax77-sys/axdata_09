...
를 입력하면 **21종 아트 요소 · 캐릭터 시트 · 쇼케이스 영상**을 한 번에
...
- **21종 아트 요소 선택 생성** — 필요한 것만 체크해서 생성 (아래 카탈로그 참고)
...
| 카테고리 | 요소 |
|----------|------|
| **캐릭터 아트** | 초상화, 전신, 표정 세트(×4), 액션 포즈(×3), 도트 스프라이트, 턴어라운드(×3), 엠블럼, 이모트 세트(×5), 탈것/펫 |
| **아이템 / 장비** | 무기·방어구 아이콘(×3), 인벤토리 아이템, 스킬 아이콘(×3), 등급 프레임(×5) |
...
├── services/
│   ├── asset_catalog.py   # 21종 아트 요소 레지스트리 (단일 진실 공급원)
